...
* `▶️ replace reaction` : React with `:arrow_forward:` to make Stardust temporarily say `next reply replaces my text`.
...
The memory maintenance pattern is now one action: run `||@stardust summarize||`, then check `soul/longmemory.txt`. Summarize also cleans adjustment audit messages after longmemory is written.
