...
function parsePipeCommandText(text, isDm) {
  ...
  const visualMatch = targetedText.match(/^visual(?:\s+(requests|cancel|process|emoji|self|scene|background|thought|dream))?(?:\s*:\s*([\s\S]*))?$/i);
  if (visualMatch) {
    const visualKeyword = (visualMatch[1] || "").toLowerCase();
    return {
      kind: "visual",
      action: ["cancel", "process", "requests"].includes(visualKeyword) ? visualKeyword : "",
      outputType: ["cancel", "process", "requests"].includes(visualKeyword) ? "" : visualKeyword,
